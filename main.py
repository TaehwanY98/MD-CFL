...
if args.type == "wesad":
    ...
    train_dataset, valid_data = random_split(train_dataset, [0.6, 0.4], generator=torch.Generator().manual_seed(args.seed))
...
if __name__=="__main__":
    ...
    def server_fn(context):
        ...
        net.to(DEVICE)
        if args.mode == "fedavg":
            strategy = FedAvgServer(net= net, eval_loader=valid_loader, args=args,inplace=True, lossf=Lossf,min_fit_clients=args.client_num, min_available_clients=args.client_num, min_evaluate_clients=args.client_num)
        elif args.mode == "ccfl":
            strategy = ClusteredFedAvg(net=net, testLoader=valid_loader, args=args, inplace=False, lossf=Lossf, CosVsMaha=CosVsMaha,min_fit_clients=args.client_num, min_available_clients=10, min_evaluate_clients=args.client_num)
        elif args.mode == "mdcfl":
            strategy = ClusteredFedAvg(net=net, testLoader=valid_loader, args=args, inplace=False, lossf=Lossf, CosVsMaha=CosVsMaha,min_fit_clients=args.client_num, min_available_clients=args.client_num, min_evaluate_clients=args.client_num)
    ...
    performance_cluster.to_csv(f"./Csv/{args.mode}_{args.type}_cluster.csv")