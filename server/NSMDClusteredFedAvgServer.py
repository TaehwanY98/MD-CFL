...
def mahalanobis_distance_cal(X,Y, cov=None):
    ...
    for x, y in zip(X,Y):
        eye=np.eye(x.size)
        distance = mahalanobis(x.reshape(-1,), y.reshape(-1,), make_inv_cov(eye, x.reshape(-1,), y.reshape(-1,)))
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy | FitRes]], failures: List[Tuple[ClientProxy | FitRes] | BaseException]) -> Tuple[Parameters | None | Dict[str, bool | bytes | float | int | str]]:
        ...
        if self.inplace:
            ...
            for indx, client_params in  enumerate([parameter_to_Ndarrays(params) for params in params_of_numfeature]):
                clusters2[f"client{indx+1}"] = mahalanobis_distance_cal(client_params, aggregated_ndarrays)
            
            
            cluster_indexs = kmeans.fit_predict( [value for _, value in clusters.items()])
            cluster_indexs2 = kmeans2.fit_predict([value for _, value in clusters2.items()])
            ...
            if n1> n0:
                indexs = np.arange(len(cluster_indexs2))[cluster_indexs==1]
            else:
                indexs = np.arange(len(cluster_indexs2))[cluster_indexs==0]
            ...
            CosVsMaha["cosine_sil"].append(sum([silhouette_score(np.stack(samples), cluster_indexs) for samples in zip(*only_params)])/len(list(zip(*only_params))))
            CosVsMaha["mahalanobis_sil"].append(sum([silhouette_score(np.stack(samples), cluster_indexs2) for samples in zip(*only_params)])/len(list(zip(*only_params))))
